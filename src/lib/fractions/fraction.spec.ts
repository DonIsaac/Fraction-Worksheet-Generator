...
const selfEqualTestCases: Fraction[] = [
    new Fraction(1, 2),
    new Fraction(4, 5, true),
]

describe(".strictEq(other)", () => {
    it.each(selfEqualTestCases)
    ("Fractions always equal themselves", frac => {
        expect(frac.strictEq(frac)).toBeTruthy()
    })

    it.each(equalTestCases)("Identical fractions are strictly equal", (n, d, neg) => {
    ...
    ])("Fractions are strictly equal when both numerators are 0", (f1, f2) => {
        expect(f1.strictEq(f2)).toBeTruthy()
    })
...
describe(".eq(other)", () => {
    ...
    it.each(equalTestCases)("Identical fractions are equal", (n, d, neg) => {
        const f1 = new Fraction(n, d, neg)
        const f2 = new Fraction(n, d, neg)
...
describe(".div(other)", () => {
    ...
    describe.each([
        ...
        [new Fraction(0, 2), new Fraction(1, 2), new Fraction(0, 2)],
